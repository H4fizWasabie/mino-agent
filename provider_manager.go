...
func (m *ProviderManager) Create(session string, role ModelRole, messages []Message, maxTokens int, system string, tools []ToolDef) (*LLMResponse, error) {
	return m.callWithConfig(session, routeRole(role, messages), func(c *Client, model, reasoning string, p ProviderConfig) (*LLMResponse, error) {
		return c.createWithRouting(context.Background(), model, reasoningForRole(p, role), messages, maxTokens, system, tools, false, false, false, nil, routingForRole(p, role), "")
	})
}
...
func (m *ProviderManager) CreateJSON(session string, role ModelRole, messages []Message, maxTokens int, system string) (*LLMResponse, error) {
	return m.callWithConfig(session, routeRole(role, messages), func(c *Client, model, reasoning string, p ProviderConfig) (*LLMResponse, error) {
		return c.createWithRouting(context.Background(), model, reasoningForRole(p, role), messages, maxTokens, system, nil, false, true, false, nil, routingForRole(p, role), "")
	})
}
...
func (m *ProviderManager) CreateContext(ctx context.Context, session string, role ModelRole, messages []Message, maxTokens int, system string, tools []ToolDef) (*LLMResponse, error) {
	return m.callContextWithConfig(ctx, session, routeRole(role, messages), func(c *Client, model, reasoning string, p ProviderConfig) (*LLMResponse, error) {
		return c.createWithRouting(ctx, model, reasoningForRole(p, role), messages, maxTokens, system, tools, false, false, false, nil, routingForRole(p, role), "")
	})
}

// CreateContextNoReasoning is CreateContext with reasoning disabled —
// for one-shot synthesis (compose_message, SCR-002): reasoning models
// stream thinking as content when content would be empty.
func (m *ProviderManager) CreateContextNoReasoning(ctx context.Context, session string, role ModelRole, messages []Message, maxTokens int, system string) (*LLMResponse, error) {
	return m.callContextWithConfig(ctx, session, routeRole(role, messages), func(c *Client, model, reasoning string, p ProviderConfig) (*LLMResponse, error) {
		return c.createWithRouting(ctx, model, reasoningForRole(p, role), messages, maxTokens, system, nil, false, false, true, nil, routingForRole(p, role), "")
	})
}
...
func (m *ProviderManager) Stream(session string, role ModelRole, messages []Message, maxTokens int, system string, tools []ToolDef, onText func(string)) (*LLMResponse, error) {
	return m.callWithConfig(session, routeRole(role, messages), func(c *Client, model, reasoning string, p ProviderConfig) (*LLMResponse, error) {
		return c.createWithRouting(context.Background(), model, reasoningForRole(p, role), messages, maxTokens, system, tools, true, false, false, onText, routingForRole(p, role), "")
	})
}
...
func (m *ProviderManager) StreamContext(ctx context.Context, session string, role ModelRole, messages []Message, maxTokens int, system string, tools []ToolDef, onText func(string)) (*LLMResponse, error) {
	return m.callContextWithConfig(ctx, session, routeRole(role, messages), func(c *Client, model, reasoning string, p ProviderConfig) (*LLMResponse, error) {
		return c.createWithRouting(ctx, model, reasoningForRole(p, role), messages, maxTokens, system, tools, true, false, false, onText, routingForRole(p, role), "")
	})
}
