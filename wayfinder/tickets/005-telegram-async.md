...
- Mino can proactively send messages (not just respond) — e.g., "Abah, I noticed I'm looping"
...
### What was built
- **Proactive notifications**: Telegram observer handles `"loop"` events → sends `🔄` message to Abah mid-task ("Detected 3 repeated calls to X — continuing but trying to self-correct.")
...
### Behavior
- Abah sends "btw status" mid-loop → Mino responds with `⚡ <status>` asynchronously
