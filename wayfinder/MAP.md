...
- [CTX-022 — Memory retrieval is loop-private — expose a deterministic read surface](tickets/ctx-022-memory-external-surface.md) (open) — `remember` (entryRanking + BFS + provenance + conflict flags) exists only as an in-loop tool; an external agent (same model, 2026-08-15 session) had to scrape `memories/*.md` over SSH and re-derive a cruder retrieval. Scope: `mino remember` CLI + MCP server + localhost/auth-gated REST, all thin wrappers over `GraphMemory.Remember`, read-only by construction — writes and the brain stay loop-private.
