...
- [CTX-021 — Updater: 30s client timeout too tight for the 22MB binary download](tickets/ctx-021-updater-download-timeout.md) (open, #177) — the shared 30s `updateClient` covers both the releases-API check and the binary body download; the 22MB download died at 30s twice on the v2.8.13 deploy (rc→release promotion itself worked). Fix: a separate longer-timeout client for the download, API check stays 30s.
