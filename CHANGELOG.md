...
- Playbook validation now **rejects stages that declare human checkpoints** ("Stop here. Ask Abah.", "request approval", etc.) at create/update time: playbooks are standing orders that run without a human present — if the task needs Abah, it is not a playbook. Stages whose `## Audit` section declares `self` mark the run **self-certified** in the result and the trace, so unverified (model-judged) outputs are visible in the audit trail instead of masquerading as machine-verified.
