...
## [Unreleased]
### Added
- Wayfinder map "Context Truth" opened (docs): root cause ticket CTX-001 confirmed from the 2026-08-10 CHEM 15 incident (wholesale large-message replacement wiped method knowledge; proximity confirmation smoothed a RM 778 discrepancy; no stop signal), resolved CTX-002 (head/tail preview, #145), frontier CTX-003..006 (verification discipline, working-state persistence, cancel-intent recognition, degeneration guard). (Why: the incident produced a measured, reproduced failure chain — the map is the contract the four open tickets are held to, in the same format as the VFY/SCH maps.)
- Session working note (implements CTX-004, closes #146): a per-session `session_notes` SQLite row is appended by the harness (every bash command, mechanically, 200-char cap) and by the model (`note_session` tool, now essential) and injected at the start of the next turn, bounded at 1500 chars with head+tail truncation. (Why: on 2026-08-10 Mino re-hunted the procura DB path three times in one session and started the CHEM 15 turn at a different project's database — turn N+1 did not inherit turn N's established paths, methods, or the open 20,073.26-vs-20.8k discrepancy. The note is ephemeral per-session working state, distinct from `save_note`'s durable graph; the harness part is judgment-free so it cannot be forgotten like the model-written `remember` note at 22:54.)
- Degeneration guard counts total parse failures per turn (implements CTX-006, closes #147): the #24 circuit breaker's streak counter is now a per-turn total — the reset on successful calls is removed — so a model alternating malformed markers and successes (2026-08-10 CHEM 15: 9 failures at iters 4, 11-14, 16, 24-26, never 6 consecutive) aborts at 6 total instead of burning to the cap. (Why: the alternation pattern made the consecutive-only guard blind; the shipped 6-threshold is preserved so recovering turns are untouched — the 3-failure-then-done test still completes.)
- Cancel-intent recognition (implements CTX-005, closes #148): `isStopMessage` matches natural cancel phrasings anywhere (its fine, nevermind, ill do it myself, ill get this data, forget it, dont bother, lets drop it); after stripping phrases and glue words, a substantive remainder means the message is a doubt/question and the turn proceeds. Leading stop/cancel/halt unchanged; rhetorical "never mind?" still stops. (Why: 2026-08-10's "Its fine then, ill get this data myself" matched no stop pattern, so the CHEM 15 turn ran to the cap instead of stopping; the doubt/cancel hybrid now runs as a cheap turn because CTX-002/004 hold the facts.)
- Number verification discipline in the system prompt (implements CTX-003, closes #149): a user-named value that differs from a computed one must be answered with BOTH numbers and the gap — never "essentially correct" — and "verified" is only claimed from a source-of-truth definition (app source/API, or the schema column defining the filter), never an invented query landing nearby. (Why: 2026-08-10 Mino smoothed a real RM 778 discrepancy — chart 20,851.69 vs computed 20,073.26 — as "your memory of ~20.8k is essentially correct", and built an item list on the invented method; the failure class is VFY, now stated in the static prompt.)
### Changed
