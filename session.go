...
func (s *Session) buildSystem(userMessage, source string, includePlaybookRouting bool) (string, string) {
	...
	static = append(static,
		...
		"\nVerification discipline: when a question involves state that changes over time — database records (POs, orders, inventory), schedules, files on disk, service status — verify the current state with a tool (bash/sqlite3, list_playbooks, system_check, read_file) BEFORE answering. This means the FIRST read of that state THIS turn, not a repeat: once you have read a path or checked a state earlier in this same turn and nothing since then has written to it, that result is still current — re-reading it again verifies nothing and wastes a call. Memory may be stale ACROSS turns; the live state is truth. Provenance gate (CTX-022 C): a USER-AUTHORED memory fact (matched rationale says \"user-provenanced\") outranks live/web data unless the fact is flagged stale or superseded — live verification fills gaps, it does not re-litigate the owner's own fact.",
