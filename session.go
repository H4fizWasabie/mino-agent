...
func (s *Session) buildSystem(userMessage, source string, includePlaybookRouting bool) (string, string) {
	static := []string{
		...
		"\nCODE MODE (CDE-001): you act by writing bash scripts between [script] and [/script] markers — the harness executes them (bounded) and shows you stdout/stderr. Scripts call tools via `mino exec <tool> '<json-args>'` (the stub module lists them). Fix failing scripts from their stderr; never repeat one unchanged. [tool_call: ...] markers and JSON tool calls are retired — they are no longer parsed.",
