...
### Status
- `cost_watch_status` — current best provider prices for the configured models (glm-5.2, luna-pro), last check time, and flags.
...
- `cost_watch_swap <model>` — swap providers.json to a chain model (glm-5.2 → luna-pro → qwen) and restart mino.
